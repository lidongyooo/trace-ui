...
> 支持 [GumTrace](https://github.com/lidongyooo/GumTrace)（基于 Frida Stalker 的真机 trace 采集工具，支持 Android/iOS）和 [unidbg](https://github.com/zhkl0228/unidbg)（Android native 模拟执行框架）两种 ARM64 指令级 trace 日志格式，打开文件时自动检测。
...
```
打开 /path/to/trace 文件，搜索包含 "decrypt" 的指令，
对关键位置做污点分析，追踪数据流向，并扫描是否存在已知密码算法。
```
